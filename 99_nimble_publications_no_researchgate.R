# 99_nimble_publications_no_researchgate.R
# using nimble to run Bayesian model of publication counts
# sensitivity analysis without researchgate in response to referee
# June 2024
seed = rep(0, 2)
...
matrix_count = mutate(analysis_ready_imputed[[imp]],
                      scholar = scholar,
                      scopus = scopus) %>%
  dplyr::select(scholar, scopus) # this ordering must be remembered
...
matrix_offset = mutate(analysis_ready_imputed[[imp]],
  ...
  dplyr::select(scholar, scopus) # this ordering must be remembered
# matrix of baseline
matrix_baseline = dplyr::select(analysis_ready_imputed[[imp]], b_scholar, b_scopus) %>%
  mutate(b_scholar = b_scholar + 0.5,
         b_scopus = b_scopus + 0.5) %>% # because of zero
  as.matrix() %>%
  log2() # 
...
# selected chains
index = which(colnames(mcmc_out_errors$samples$chain1) %in% c('alpha[1]','alpha[2]','beta','gamma','tau.person'))
...
residuals = mutate(analysis_ready_imputed[[imp]], 
  ...
  pivot_longer(cols=c('scholar', 'scopus'),
               names_to = 'database',
               values_to = 'observed') %>%
...
results_no_researchgate = list()
results_no_researchgate$covmat = covmat
results_no_researchgate$model_fit = model_fit
results_no_researchgate$table = table
results_no_researchgate$residuals = residuals
results_no_researchgate$chains = chains
