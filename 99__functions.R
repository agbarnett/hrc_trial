...
add_treatment_censored = function(in_random, # data frame with randomisation (treatment) information
  ...
  if(check_plot == TRUE){
    ...
    bplot = ggplot(data=check, aes(x=time, y=n, fill=funded))+
      ...
      scale_fill_manual(NULL, values=c('dodgerblue','darkseagreen','grey80'))+ # matching colours used elsewhere
      theme_bw()+
      theme(legend.position = 'top')+
      ylab('Number of researchers')+
      xlab(xlab)
    bplot
    jpeg('figures/number_treated_by_timex.jpg', width=7, height=5, units='in', res=500, quality=100)
    print(bplot)
    dev.off()
  } # end of if
