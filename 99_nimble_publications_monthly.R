# 99_nimble_publications_monthly.R
...
codeErrors <- nimbleCode({
  for (i in 1:N) {
    for(j in 1:C){
      ...
      log(mu[i,j]) <- log(offset[i,j]) + alpha[j] + zeta*january[i] + r_int[person[i]] + (beta*baseline[i,j]) + (gamma*treatment[i]) + errors[i,j]
    }
...
matrix_count = mutate(analysis_ready_monthly_imputed[[imp]],
                      scopus = scopus,
                      researchgate = researchgate) %>%
  dplyr::select(scopus, researchgate) # this ordering must be remembered
matrix_count = as.matrix(matrix_count)
# matrix of baseline
matrix_baseline = dplyr::select(analysis_ready_monthly_imputed[[imp]], b_scopus, b_researchgate) %>%
  mutate(b_scopus = b_scopus + 0.5,
         b_researchgate = b_researchgate + 0.5) %>% # because of zero
  as.matrix() %>%
  log2() # transform
# matrix of offsets
matrix_offset = mutate(analysis_ready_monthly_imputed[[imp]],
                       scopus = denom_scopus,
...
constantsErrors <- list(N = nrow(analysis_ready_monthly_imputed[[imp]]),
                        offset = matrix_offset, # adjust for final year that is under a year
                        R = R,
                  treatment = as.numeric(analysis_ready_monthly_imputed[[imp]]$funded == 'Funded'),
                  zeros = c(0,0), 
                  person = as.numeric(as.factor(analysis_ready_monthly_imputed[[imp]]$number)),
                  P = length(unique(analysis_ready_monthly_imputed[[imp]]$number)),
                  baseline = matrix_baseline,
                  january = as.numeric(analysis_ready_monthly_imputed[[imp]]$month == 1),
                  C = 2)
...
# parameters to store
parmsErrors = c('r_int','tau.person','beta','alpha','gamma','zeta','Sigma', 'mu')
...
# selected chains
index = which(colnames(mcmc_out$samples$chain1) %in% c('alpha[1]','alpha[2]','beta','gamma','tau.person'))
...
fitted = filter(table, str_detect(rowname, pattern='^mu')) %>% # model predictions
  ...
  mutate(database = case_when(
    ...
    row = as.numeric(row))
#
residuals = mutate(analysis_ready_monthly_imputed[[imp]], 
                   row = 1:n(),
...
results_monthly$covmat = covmat
results_monthly$model_fit = model_fit
results_monthly$table = table
results_monthly$residuals = residuals
results_monthly$chains = chains
