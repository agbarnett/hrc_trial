...
codeErrors <- nimbleCode({
  for (i in 1:N) {
    for(j in 1:C){
      ...
      mu[i,j] <- alpha[j] + zeta*january[i] + r_int[person[i]] + (beta*baseline[i,j]) + (gamma*treatment[i]) + errors[i,j]
    }
...
# parameters to store
parmsErrors = c('r_int','tau','tau.person','beta','alpha','gamma','zeta','Sigma', 'mu')
