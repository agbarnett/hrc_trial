...
```{r, include=FALSE}
#file = c('bayes', 'main_publication_counts_loge') # alternative version with base e log for reviewer
...
# Without researchgate

Here we examine the effect of funding on yearly publication counts without researchgate. 

```{r, include=FALSE}
file = c('bayes', 'main_publication_no_researchgate')
outfile = make_file(file, only_consent = only_consent)
if(outfile$exists == FALSE){
  # model choices:
  pilot = FALSE # run big model
  ...
  results = list() # list to store results
  for (imp in 1:n_impute){
    source('99_nimble_publications_no_researchgate.R')
    results[[imp]] = results_no_researchgate
  }
  save(results, pilot, file = outfile$file)
}
if(outfile$exists == TRUE){
...
results_no_researchgate = results # rename
```

### Table of estimates (yearly publication data - without researchgate)

The table below uses the estimates from a single imputation.

```{r}
to_table = filter(results_no_researchgate[[1]]$table, 
    str_detect(parameter, pattern='beta|gamma')) %>% # do not show intercept
  mutate(term = parameter_rename(parameter),
  pvalue = format.pval(pvalue, digits=2, eps=0.0001), # not in imputed data
    mean = exp(mean), # rate ratio
...
to_table = filter(results_citations$table, 
    str_detect(parameter, pattern='beta|gamma')) %>% # do not show intercept
  mutate(term = parameter_rename(parameter, which_est = 'citations'),
  pvalue = format.pval(pvalue, digits=2, eps=0.0001),
    mean = exp(mean), # rate ratio
           lower = exp(lower),
           upper = exp(upper),
...
splot_main = ggplot(data=time_varying, aes(x=years_since, y=fitted, ymin=lwr, ymax=upr, col=factor(funded)))+
...
The shaded areas are 80% confidence intervals. 
...
to_table = filter(results_citations_per_paper$table, 
    str_detect(rowname, pattern='beta|gamma')) %>% # do not show intercept
  mutate(term = parameter_rename(rowname, which_est = 'citations_per_paper'),
...
to_table = filter(results_citations_per_paper_time_varying$table, 
    str_detect(parameter, pattern='beta|gamma|delta|lambda|slope.diff')) %>% # do not show intercept
  mutate(term = parameter_rename(parameter, which_est ='citations_per_paper'),
  pvalue = format.pval(pvalue, digits=2, eps=0.0001),
  mean = roundz(mean, 1), # rounding
         lower = roundz(lower, 1),
         upper = roundz(upper, 1),
...
to_table = filter(results_altmetric$table, 
    str_detect(parameter, pattern='beta|gamma')) %>% # do not show intercept
  mutate(term = parameter_rename(parameter, which_est = 'altmetric'),
  pvalue = format.pval(pvalue, digits=2, eps=0.0001),
    mean = roundz(mean, 1), # rounding
         lower = roundz(lower, 1),
         upper = roundz(upper, 1),
