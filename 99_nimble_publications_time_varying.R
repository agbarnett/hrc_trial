...
codeErrors <- nimbleCode({
  for (i in 1:N) {
    ...
    for(j in 1:C){
      ...
      log(mu[i,j]) <- log(offset[i,j]) + alpha[j] + r_int[person[i]] + (beta*baseline[i,j]) + trt[i] + control[i] + errors[i,j]
    }
...
constantsErrors <- list(N = nrow(analysis_ready_imputed[[imp]]),
                        ...
                        R = R,
                  T = max(analysis_ready_imputed[[imp]]$years_since),
                  years_since = analysis_ready_imputed[[imp]]$years_since - 2, # centre by 2 years
                  ...
                  baseline = matrix_baseline, # base 2 for doubling
                  C = 3)
...
# parameters to store
parmsErrors = c('r_int','tau.person','beta','alpha','gamma', 
                'delta', 'lambda', 'Sigma', 'mu', 'diff', 'slope.diff')
...
fitted = filter(table, str_detect(rowname, pattern='^mu')) %>% # model predictions
  ...
  mutate(database = case_when(
    ...
    row = as.numeric(row))
#
residuals = mutate(analysis_ready_imputed[[k]], 
                   row = 1:n(),
...
results_time_varying$chains = chains
